...
def main():
    ...
    ip_header  = create_ip_header(host, host)
    tcp_header = create_tcp_header(message, host, host)
    packet     = create_packet(ip_header, tcp_header, message)

    try:
        # MODIFICATION : SOCK_STREAM → SOCK_RAW
        s = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_RAW)
        s.sendto(packet, (host, 0))
        print('message JSON envoye au serveur')
        s.close()
